  ...
  methods: {
    ...
    async fetchSpots() {
      try {
        ...
        const response = await axios.get(`${process.env.APICALL}/api/dashboard/parkingPlaces/parkingSpots/show/${id}`, {
          headers: {
    ...
    async deleteParkingSpot(id) {
      if (confirm("Are you sure you want to delete this Spot?")) {
        const token = this.$auth.strategy.token.get();
        await axios.delete(`${process.env.APICALL}/api/dashboard/parkingPlaces/parkingSpots/delete/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
