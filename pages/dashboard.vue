...
export default {
  ...
  async asyncData({$axios}) {
    const response = await $axios.$get(`https://smart-parking-project.herokuapp.com/api/dashboard`);
    return {
  ...
  methods: {
    ...
    async updateUser() {
      ...
      await axios.patch(`https://smart-parking-project.herokuapp.com/api/dashboard/users/${userID}`, {
    ...
    fetchUsers() {
      const token = this.$auth.strategy.token.get()
      axios.get(`https://smart-parking-project.herokuapp.com/api/dashboard/users`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      }).then(response => {
    ...
    deleteUser(userId) {
      if (confirm("Are you sure you want to delete this user?")) {
        const token = this.$auth.strategy.token.get()
        axios.delete(`https://smart-parking-project.herokuapp.com/api/dashboard/users/delete/${userId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
