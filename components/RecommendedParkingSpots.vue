...
<!--        <div>Distance: {{ parkingSpot.distance.toFixed(2) }} km</div>-->
<!--      </li>-->
<!--    </ul>-->
<!--  </div>-->
<!--</template>-->
...
<!--      console.log("RecommendedParkingSpots: " + JSON.stringify(parkingSpots));-->
      ...
      <ul>
        <li v-for="parkingSpot in recommendedParkingSpots" :key="parkingSpot.id">
          <div class="mb-1 rounded-md border border-gray-300 p-2 hover:bg-gray-100 active:bg-gray-200 cursor-pointer" @click="showParkingPlacePreview(parkingSpot)">
            {{parkingSpot.name}} Free Spots: {{parkingSpot.free_spots}}
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    getRecommendedParkingSpots() {
      ...
      // return recommendedSpots;
      return sortedParkingSpots.slice(0,4)
    },
