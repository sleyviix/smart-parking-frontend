<template>
  <div
    ...
    <div class="flex pb-6 pt-10 lg:pt-0 items-center">
      <div class="w-5/6 text-4xl tracking-tight font-extrabold text-gray-700">{{ parkingPlace.name }}</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 mt-6">
      ...
      <div>
        ...
        <div class="relative flex items-start mt-2" v-for="(price, attribute) in attributes" :key="attribute">
          ...
          <div class="ml-3 text-sm flex justify-between flex-grow">
            <label :for="attribute" class="font-medium text-gray-700">{{attribute}}</label>
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-3 mt-2">
          ...
          >
            <p class="text-gray-700 text-lg font-bold">size.name</p>
...
export default {
  ...
  watch: {
    parkingPlace: function (newValue, oldValue) {
  ...
  methods: {
    ...
    async filterSpots() {
      ...
      const {data} = await this.$axios.get(`http://localhost:8000/api/parkingPlace/${this.parkingPlace.id}/spots?${this.buildFilterUrl()}`)
